...
User's request to enhance:`;
    ...
    // Call Groq API for enhancement with retry logic
    console.log('Calling Groq API with prompt:', originalPrompt.trim());
    ...
    while (retryCount < maxRetries) {
      try {
        ...
        enhancedPrompt = chatCompletion.choices[0]?.message?.content;
        console.log('Groq API response received successfully');
        break; // Success, exit retry loop
      } catch (aiError) {
        retryCount++;
        console.error(`Enhancement error (attempt ${retryCount}/${maxRetries}):`, aiError.message);
        
        // Check if it's a rate limit or server error
        if ((aiError.message.includes('429') || aiError.message.includes('503')) && retryCount < maxRetries) {
          console.log(`Retrying in ${retryCount * 2} seconds...`);
          await new Promise(resolve => setTimeout(resolve, retryCount * 2000)); // Wait 2s, 4s, 6s
          continue;
        }
        ...
        if (retryCount >= maxRetries) {
          return NextResponse.json(
            { error: 'AI servers are temporarily busy. Please try again in a few minutes.' },
            { status: 503, headers }
          );
        }
